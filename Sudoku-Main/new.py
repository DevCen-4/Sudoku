...
# Hàm gợi ý cho người chơi và thông báo tại sao lại điền như vậy
def FindHint(filename='Result.txt'):
    with open(filename, 'r') as file:
        for line in file:
            ...
            if numbers[0] == 1:
                s = "The last free cell"
            if numbers[0] == 2:
                s = "The last remaining cell in row"
            if numbers[0] == 3:
                s = "The last ramaining cell in column"
            if numbers[0] == 4:
                s = "The last ramaining cell in square"
...
# Hàm vẽ các nút và số cần thiết lên cửa sổ trò chơi
def draw_number_and_new_game_button(window):
    ...
    img_return = pygame.image.load('Image/hint.png').convert_alpha()
...
def draw_numbers(screen, selected=(0,0)):
...
def check_full():
    ...
    for row in range(9):
        for col in range(9):
            if control_grid[row][col] != 0 and control_grid[row][col] != -4:
                count += 1
    return count == 81
...
def draw_fraction(screen):
    ...
    font = pygame.font.SysFont('Arial', 20,)  # Font chữ, kích thước chữ
    color = (45, 72, 97)  # Màu chữ (đen)
    global num_error  # Biến toàn cục

    # Tạo chuỗi dạng "x/3", trong đó x là count_false_number
    text = f"Error: {num_error}/3"
    ...
    text_rect.topright = (1670-70, 23)  # Căn góc phải trên, cách mép 10px

    # Vẽ chuỗi lên screen
    screen.blit(rendered_text, text_rect)

# Thực hiện tính toán thời gian chơi game
# Chơi game quá 180 phút / ngày có hại cho sức khỏe
def check_time(screen,timer_running = False,start_time = 0 ,elapsed_time = 0 ):
    if timer_running:
        elapsed_time = (pygame.time.get_ticks() - start_time) / 1000  # Đổi sang giây
    ...
    time_text = font.render(f"Time: {minutes:02}:{seconds:02}", True,color)
    screen.blit(time_text, (1670-300, 23))
